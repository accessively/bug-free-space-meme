const fs = require('fs');
const path = require('path');

// Create a simple SVG logo as placeholder
const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg width="128" height="128" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect width="128" height="128" rx="8" fill="#4F46E5"/>
  <text x="64" y="72" font-size="32" font-weight="bold" fill="white" text-anchor="middle" font-family="Arial">A</text>
</svg>`;

const outputPath = path.join(__dirname, 'public', 'images', 'logo.png');
fs.mkdirSync(path.dirname(outputPath), { recursive: true });
fs.writeFileSync(outputPath.replace('.png', '.svg'), svg);
console.log('Placeholder SVG created at', outputPath.replace('.png', '.svg'));
